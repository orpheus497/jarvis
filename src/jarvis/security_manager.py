...
class SecurityManager:
    ...
    async def verify_challenge_response(
        self, ip_address: str, response: bytes, secret: bytes
    ) -> bool:
        """
    ...
    async def record_connection_attempt(self, ip_address: str, success: bool = True):
        ...
        async with self.lock:
            ...
            self.connection_attempts[ip_address] = [
                t for t in self.connection_attempts[ip_address] if now - t < window
            ]

            if success:
                self.active_connections[ip_address] = self.active_connections.get(ip_address, 0) + 1
    ...
    async def _record_failed_attempt(self, ip_address: str):
        ...
        if len(attempts) >= IP_BAN_THRESHOLD:
            await self.ban_ip(ip_address, duration=IP_BAN_DURATION)
            self._log_event(
                ip_address, "AUTO_BANNED", f"Banned after {len(attempts)} failed attempts"
            )
    ...
    async def check_rate_limit(self, ip_address: str, limit_type: str = "message") -> bool:
        ...
        async with self.lock:
            ...
            if len(recent) >= limit:
                ...
                self._log_event(
                    ip_address, "RATE_LIMIT_EXCEEDED", f"{limit_type}: {len(recent)}/{limit}"
                )
                return False
    ...
    async def ban_ip(self, ip_address: str, duration: int = IP_BAN_DURATION):
        ...
        async with self.lock:
            ...
            logger.warning(
                f"IP banned: {ip_address} for {duration}s " f"(until {time.ctime(unban_time)})"
            )
            self._log_event(ip_address, "BANNED", f"Temporary ban for {duration}s")
    ...
    def _log_event(self, ip_address: str, event_type: str, details: str):
        """Log security event."""
        event = SecurityEvent(
            timestamp=time.time(), ip_address=ip_address, event_type=event_type, details=details
        )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        # Count active bans
        active_bans = sum(1 for unban_time in self.temp_bans.values() if unban_time > now)

        # Count recent events by type
        recent_window = 3600  # Last hour
        recent_events = [e for e in self.events if now - e.timestamp < recent_window]
        ...
        top_ips = sorted(self.connection_attempts.items(), key=lambda x: len(x[1]), reverse=True)[
            :10
        ]
        ...
        return {
            ...
            "top_connection_ips": [
                {"ip": ip, "attempts": len(attempts)} for ip, attempts in top_ips
            ],
        }

    async def get_recent_events(
        self, count: int = 50, event_type: Optional[str] = None
    ) -> list[SecurityEvent]:
        """
    ...
    async def cleanup_expired(self):
        ...
        async with self.lock:
            ...
            # Remove expired bans
            expired_bans = [ip for ip, unban_time in self.temp_bans.items() if unban_time <= now]
            ...
            for ip in list(self.connection_attempts.keys()):
                self.connection_attempts[ip] = [
                    t for t in self.connection_attempts[ip] if now - t < window
                ]
            ...
            for ip in list(self.message_counts.keys()):
                self.message_counts[ip] = [t for t in self.message_counts[ip] if now - t < window]
            ...
            expired_challenges = [
                ip
                for ip, (_, timestamp) in self.active_challenges.items()
