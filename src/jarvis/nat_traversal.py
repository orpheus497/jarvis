...
class NATTraversal:
    ...
    def setup_upnp_mapping(self, local_port: int, external_port: int = None,
                          protocol: str = 'TCP', description: str = 'Jarvis P2P',
                          duration: int = 3600) -> Optional[Tuple[str, int]]:
        """
        ...
        try:
            ...
            logger.info(f"Adding UPnP port mapping: {external_port} -> {local_ip}:{local_port} ({protocol})")
    ...
    def cleanup_mappings(self):
        ...
        for mapping_key, mapping_info in list(self.mapped_ports.items()):
            self.remove_upnp_mapping(
                mapping_info['external_port'],
                mapping_info['protocol']
            )
    
    def get_connection_strategy(self, target_nat_type: Optional[NATType] = None) -> ConnectionStrategy:
        """
        ...
        if self.nat_type in [NATType.FULL_CONE, NATType.RESTRICTED_CONE, 
                             NATType.PORT_RESTRICTED_CONE]:
            if target_nat_type in [NATType.FULL_CONE, NATType.RESTRICTED_CONE,
                                   NATType.PORT_RESTRICTED_CONE]:
