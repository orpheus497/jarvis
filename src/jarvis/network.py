...
from .connection_fsm import ConnectionState as FSMState
...
class P2PConnection:
    ...
    def _on_fsm_state_change(self, old_state: FSMState, new_state: FSMState):
        ...
        logger.debug(
            f"Connection FSM: {old_state.name} -> {new_state.name} for {self.contact.username}"
        )
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._set_state(ConnectionState.CONNECTING)
            logger.info(
                f"Attempting connection to {self.contact.username} ({self.contact.host}:{self.contact.port}) - Attempt #{self.connection_attempts}"
            )
            ...
            try:
                ...
                logger.warning(
                    f"Connection timeout to {self.contact.username} after {self.CONNECT_TIMEOUT}s"
                )
            ...
            try:
                key_len_data = await asyncio.wait_for(
                    self.reader.readexactly(4), timeout=self.HANDSHAKE_TIMEOUT
                )
                key_len = struct.unpack("!I", key_len_data)[0]
                if key_len != 32:
                    logger.error(
                        f"Invalid public key length from {self.contact.username}: {key_len}"
                    )
                ...
                peer_public_key_bytes = await asyncio.wait_for(
                    self.reader.readexactly(key_len), timeout=self.HANDSHAKE_TIMEOUT
                )
            ...
            if received_fingerprint != self.contact.fingerprint:
                logger.error(
                    f"Fingerprint mismatch for {self.contact.username}! Possible MITM attack!"
                )
            ...
            self.session_keys = crypto.perform_key_exchange(
                self.identity.private_key, peer_public_key
            )
            ...
            try:
                header_data = await asyncio.wait_for(
                    self.reader.readexactly(Protocol.HEADER_SIZE), timeout=self.HANDSHAKE_TIMEOUT
                )
                _version, _msg_type_int, payload_length = struct.unpack("!BHI", header_data)

                # Receive the full payload
                payload_data = await asyncio.wait_for(
                    self.reader.readexactly(payload_length), timeout=self.HANDSHAKE_TIMEOUT
                )
            ...
            if msg_type != MessageType.HANDSHAKE_RESPONSE:
                logger.error(
                    f"Expected HANDSHAKE_RESPONSE from {self.contact.username}, got {msg_type}"
                )
    ...
    async def send_message(self, plaintext: str, message_id: str, timestamp: str) -> bool:
        ...
        if self.state != ConnectionState.AUTHENTICATED or not self.session_keys:
            logger.warning(
                f"Cannot send message to {self.contact.username}: Not authenticated (state={self.state})"
            )
            return False
    ...
    async def send_group_message(
        self, group_id: str, message_id: str, plaintext: str, timestamp: str
    ) -> bool:
        """Send encrypted group message to peer."""
        if self.state != ConnectionState.AUTHENTICATED or not self.session_keys:
            logger.warning(
                f"Cannot send group message to {self.contact.username}: Not authenticated (state={self.state})"
            )
            return False
        ...
        try:
            ...
            message = Protocol.create_group_message(
                group_id, message_id, self.my_uid, plaintext, timestamp, encrypted
            )
            ...
            try:
                await asyncio.wait_for(self.send_queue.put(message), timeout=5.0)
                logger.debug(
                    f"Group message queued for {self.contact.username} (Group: {group_id[:8]}, ID: {message_id})"
                )
                return True
            except asyncio.TimeoutError:
            ...
            logger.error(
                f"Failed to queue group message to {self.contact.username}: {e}", exc_info=True
            )
            return False
            ...
            while self.state in (ConnectionState.CONNECTED, ConnectionState.AUTHENTICATED):
                try:
                    ...
                    data = await asyncio.wait_for(
                        self.reader.read(4096), timeout=self.OPERATION_TIMEOUT
                    )
                    ...
                    while len(self.buffer) >= Protocol.HEADER_SIZE:
                        ...
                        try:
                            ...
                            logger.error(
                                f"Error handling message from {self.contact.username}: {e}",
                                exc_info=True,
                            )
                    ...
                    logger.error(
                        f"Unexpected error in receive loop for {self.contact.username}: {e}",
                        exc_info=True,
                    )
                    break
        finally:
        ...
        try:
            while self.state in (ConnectionState.CONNECTED, ConnectionState.AUTHENTICATED):
                try:
                    ...
                    logger.error(
                        f"Error in send loop for {self.contact.username}: {e}", exc_info=True
                    )
                    break
        finally:
            ...
            while self.state in (ConnectionState.CONNECTED, ConnectionState.AUTHENTICATED):
                ...
                try:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Send queue full, skipping ping for {self.contact.username}"
                        )
...
class P2PServer:
    ...
    def __init__(self, port: int, identity: crypto.IdentityKeyPair, my_uid: str, my_username: str):
        self.port = port
        self.identity = identity
        self.my_uid = my_uid
        self.my_username = my_username
    ...
    async def _handle_client(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
        ...
        try:
            ...
            # Derive session keys
            peer_public_key = crypto.IdentityKeyPair.from_public_bytes(peer_public_key_bytes)
            session_keys = crypto.perform_key_exchange(self.identity.private_key, peer_public_key)
            ...
            header_data = await asyncio.wait_for(
                reader.readexactly(Protocol.HEADER_SIZE), timeout=15
            )
            ...
            logger.debug(
                f"Received handshake from {handshake_payload.get('username')} (fingerprint: {fingerprint[:16]}...)"
            )
            ...
            if self.on_connection_callback:
                if asyncio.iscoroutinefunction(self.on_connection_callback):
                    await self.on_connection_callback(
                        reader, writer, session_keys, fingerprint, address, handshake_payload
                    )
                else:
    ...
    async def start_server(self) -> bool:
        ...
        try:
            ...
            upnp_result = self.nat_traversal.setup_upnp_mapping(
                self.listen_port, protocol="TCP", description="Jarvis P2P Messenger"
            )
            ...
            self.discovery = DiscoveryService(
                self.my_uid, self.my_username, public_key_b64, fingerprint
            )
            self.discovery.on_peer_discovered = self._handle_peer_discovered
            self.discovery.on_peer_lost = self._handle_peer_lost

            # Start discovery service
            discovery_started = await self.discovery.start(self.public_port or self.listen_port)
    ...
    async def _connection_manager_loop(self):
        ...
        try:
            while self.running:
                ...
                if (
                    asyncio.get_event_loop().time() - last_reconnect_attempt
                ...
                async with self._lock:
                    disconnected = [
                        ...
                        or conn.state == ConnectionState.ERROR
                    ]
    ...
    def _handle_group_message(
        ...
        if self.on_group_message_callback:
            if asyncio.iscoroutinefunction(self.on_group_message_callback):
                asyncio.create_task(
                    self.on_group_message_callback(
                        group_id, sender_uid, content, message_id, timestamp
                    )
                )
            else:
    ...
    async def _handle_incoming_connection(
        ...
        async with self._lock:
            ...
            if not contact:
                ...
                await writer.wait_closed()
                logger.warning(
                    f"Rejected connection from unknown fingerprint: {fingerprint[:16]}..."
                )
        ...
        for msg in queued:
            try:
                ...
                success = await connection.send_message(
                    message_data["content"], message_data["message_id"], message_data["timestamp"]
                )
